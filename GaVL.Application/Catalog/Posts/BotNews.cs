...
namespace GaVL.Application.Catalog.Posts
{
    ...
    public class BotNews : IBotNews
    {
        ...
        public BotNews(AppDbContext db, IRedisService redis)
        {
            _db = db;
            _redis = redis;
        }
        private static readonly string ApiKey = Environment.GetEnvironmentVariable("AIzaSyC5d4GDVCYBvgp2q0cp1kIMSrsq83iMzyA") ?? "AIzaSyC5d4GDVCYBvgp2q0cp1kIMSrsq83iMzyA";
        private static readonly string GeminiEndpoint = $"https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-preview:generateContent?key={ApiKey}";
        private static readonly List<string> RssSources = new()
        {
        ...
        private static readonly Guid BotUserId = Guid.Parse("019bdb9d-07b5-71a4-a310-acc7670b4ea6");
        private static readonly int DefaultCategoryId = 1;
        public async Task<string> Run()
        {
            ...
            if (randomNews == null)
            {
                Console.WriteLine("Lỗi: Không tìm thấy tin nào trong 24h qua.");
            ...
            string prompt = $@"
                    ...
                    {{
                        ""ImageUrl"": ""link ảnh đại diện cho bài viết, có thể lấy og:image từ bài viết, nếu không có thì hãy lấy 1 hình ảnh có liên quan tới nội dung"",
            ...
            foreach (var url in RssSources)
            {
                try
                {
                    ...
                    foreach (var item in feed.Items)
                    {
                        ...
                        if (item.PublishDate > DateTimeOffset.Now.AddHours(-24))
                        {
        ...
        static async Task<string> GenerateContentWithGemini(string promptText)
        {
            using var client = new HttpClient();
            var requestBody = new
            {
        ...
        async Task SavePostToDatabase(GeneratedArticleDto dto)
        {
            ...
            var newPost = new Post
            {
                ...
                MainImage = dto.ImageUrl, // Link ảnh lấy từ RSS gốc
